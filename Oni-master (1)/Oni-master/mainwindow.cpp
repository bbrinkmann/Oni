...
Engine MainWindow::createEngine() {
    Card *card[5];
    QList<QList<int*>> *pieces1,*pieces2;
    int p1[5][2] ,p2[5][2];
    for(int i=0;i<game->getPieces()->size();i++){
        if(game->getPieces()->at(i)->getOwner()==1){
            if(game->getPieces()->at(i)->getType()=='M'){
                p1[0][0]=game->getPieces()->at(i)->getCol();
                p1[0][1]=game->getPieces()->at(i)->getRow();
            }
            else {
                QList<int*> p;
                int *s1=new int;
                int *s2=new int;
                *s1=game->getPieces()->at(i)->getCol();
                *s2=game->getPieces()->at(i)->getRow();
                p.append(s1);
                p.append(s2);
                pieces1->append(p);
                p.clear();
            ...
            if(game->getPieces()->at(i)->getType()=='m'){
                p2[0][0]=game->getPieces()->at(i)->getCol();
                p2[0][1]=game->getPieces()->at(i)->getRow();
            }
            else {
                QList<int*> p;
                int *s1=new int;
    ...
    for(int i=1;i<5;i++){
        if(i>pieces1->size()){
            p1[i-1][0]=*pieces1->at(i-1).at(0);
            p1[i-1][1]=*pieces1->at(i-1).at(1);
        }else {
            p1[i-1][0]=-1;
            p1[i-1][1]=-1;
        }
        if(i>pieces2->size()){
            p2[i-1][0]=*pieces2->at(i-1).at(0);
            p2[i-1][1]=*pieces2->at(i-1).at(1);
        }else {
            p2[i-1][0]=-1;
            p2[i-1][1]=-1;
        }
    }
    card[4]=game->getSlotsGrid()->at(0).at(0)->getCard();
    card[0]=game->getSlotsGrid()->at(1).at(0)->getCard();
    card[1]=game->getSlotsGrid()->at(1).at(1)->getCard();
    card[2]=game->getSlotsGrid()->at(2).at(0)->getCard();
    card[3]=game->getSlotsGrid()->at(2).at(1)->getCard();
    Engine oni(p1,p2,card);
    return oni;
}

void MainWindow::engineTurn(){
    if(game->getFirstPlayersTurn()){
        if(ui->actionRedEasy->isChecked()||ui->actionRedMedium->isChecked()||ui->actionRedHard->isChecked()){
            Engine oni=createEngine();
            CardSlot *cardslot;
            int col,row,newcol,newrow;
            player1.VI(oni,true,engineSize);
            col=player1.getColFromChoice();
            row=player1.getRowFromChoice();
            newcol=col+player1.getCard()->getColFromChoice(player1.getCardChoice());
            newrow=row+player1.getCard()->getRowFromChoice(player1.getCardChoice());
            game->setPickedUpPiece(game->getBoard()->at(col).at(row)->getPiece());
            game->getBoard()->at(newcol).at(newrow)->linkPiece(game->getPickedUpPiece());
            game->setFirstPlayersTurn(!game->getFirstPlayersTurn());
            game->getWindow()->saveTurnInNotation();
            game->getBoard()->at(col).at(row)->setPieceType(' ');
            if(game->getSlotsGrid()->at(1).at(0)->getCard()==player1.getCard())cardslot=game->getSlotsGrid()->at(1).at(0);
            else cardslot=game->getSlotsGrid()->at(1).at(1);
            game->switchCards(cardslot);
        }
    }else{
        if(ui->actionRedEasy->isChecked()||ui->actionRedMedium->isChecked()||ui->actionRedHard->isChecked()){
            Engine oni=createEngine();
            CardSlot *cardslot;
            int col,row,newcol,newrow;
            player2.VI(oni,false,engineSize);
            col=player2.getColFromChoice();
            row=player2.getRowFromChoice();
            newcol=col-player2.getCard()->getColFromChoice(player2.getCardChoice());
            newrow=row-player2.getCard()->getRowFromChoice(player2.getCardChoice());
            game->setPickedUpPiece(game->getBoard()->at(col).at(row)->getPiece());
            game->getBoard()->at(newcol).at(newrow)->linkPiece(game->getPickedUpPiece());
            game->setFirstPlayersTurn(!game->getFirstPlayersTurn());
            game->getWindow()->saveTurnInNotation();
            game->getBoard()->at(col).at(row)->setPieceType(' ');
            if(game->getSlotsGrid()->at(2).at(0)->getCard()==player2.getCard())cardslot=game->getSlotsGrid()->at(2).at(0);
            else cardslot=game->getSlotsGrid()->at(2).at(1);
            game->switchCards(cardslot);
        }
    }
}
...
void MainWindow::on_actionRedEasy_triggered() {
    ui->actionRedHard->setChecked(false);
    ui->actionRedMedium->setChecked(false);
    engineSize=2;
    engineTurn();
}
...
void MainWindow::on_actionRedMedium_triggered() {
    ui->actionRedHard->setChecked(false);
    ui->actionRedEasy->setChecked(false);
}
...
void MainWindow::on_actionBlueEasy_triggered() {
    ui->actionBlueHard->setChecked(false);
    ui->actionBlueMedium->setChecked(false);
}

void MainWindow::on_actionBlueMedium_triggered() {
    ui->actionBlueHard->setChecked(false);
    ui->actionBlueEasy->setChecked(false);
}
